...
class track_object_sample():
    ...
    def draw(self,frame,tracker,d=80):
        ...
        if start_flag == True:
            ...
            x = (int((box.left() + box.right()) / 2) - 327) / 607
            y = (int((box.top() + box.bottom()) / 2) - 234) / 607
...
class PyQt_MVC_Main(QMainWindow):
    ...
    def find_corner(self,gray, img,data,cam2end_data):
        ...
        thedaX = atan2(end_to_base[2, 1], end_to_base[2, 2]) * 180 / np.pi
        thedaY = atan2(end_to_base[2, 0] * -1,
                       sqrt(end_to_base[2, 0] ** 2 + end_to_base[2, 2] ** 2)) * 180 / np.pi
        thedaZ = atan2(end_to_base[1, 0], end_to_base[0, 0]) * 180 / np.pi
        print('3.1 驗證末端相對於基座的變換矩陣 = (當前手臂座標) ',
              np.dot(end_to_base, [0, 0, 0, 1]))
    ...
    def cam(self):
        ...
        with open('camera_to_end_20230414_145306.yaml', 'r') as f:
        ...
        try:
            with dai.Device(pipeline) as device:
                ...
                COLOR_GREEN = "\033[92m"
                COLOR_RESET = "\033[0m"
                message = f"{COLOR_GREEN}[Tips]: Detected OAK-D!{COLOR_RESET}\n"
                ...
                while True:
                    ...
                    #物件追蹤
                    if trace_object:
                        X, Y = track_object_sample.draw(self, frame, tracker, d=60)
                        print(X,Y)
            ...
            # print('nope')
            try:#開啟intel realsense d435i
                # 載D435i內參矩陣
                COLOR_RED = "\033[91m"
                COLOR_GREEN = "\033[92m"
                COLOR_RESET = "\033[0m"
                message = f"{COLOR_GREEN}[Tips]: Detected D435i!{COLOR_RESET}\n"
                sys.stdout.write(message)
                sys.stdout.flush()

                # print("Detected D435i")
                with open('D435i_calibration.yaml', 'r') as f:
                    data = yaml.load(f.read(), Loader=yaml.FullLoader)
                # 載入相機相對於末端的變換矩陣 - 手眼標定結果
                with open('D435i_to_End.yaml', 'r') as f:
                    cam2end_data = yaml.load(f.read(), Loader=yaml.FullLoader)
                ...
                while True:
                    ...
                    frame = cv2.resize(color_image, (640, 480))  # 改變尺寸和視窗相同
                    frame = cv2.cvtColor(frame, cv2.COLOR_BGR2RGB)  # 轉換成 RGB
                    if take_picture:
                        take_picture = False
                        path = f'{id}/img'
                    ...
                    # 物件追蹤
                    if trace_object:
                        X, Y = track_object_sample.draw(self, frame, tracker, d=190)
                        # print(X, Y)
                        ...
                        if coords != []:
                            ...
                            end_to_base = PyQt_MVC_Main.pose_robot(self, rx, ry, rz, ax, ay, az)
                            RT_chess_to_base = end_to_base @ cam2end
                            print("相機座標至末端座標值 : ", RT_chess_to_base @ [X, Y, 20, 1])
                        # threading.Thread(target=PyQt_MVC_Main.move_by_cam, args=(self, X, Y)).start()
                        # trace_object = False
                ...
                while True:
                    ...
                    # 物件追蹤
                    if trace_object:
                        X, Y = track_object_sample.draw(self, frame, tracker, d=20)
                        # print(X, Y)
                        # if X ** 2 < 150 and Y ** 2 < 150:
                        print(X, Y)
                        ...
                        if coords != []:
                            ...
                            end_to_base = PyQt_MVC_Main.pose_robot(self, rx, ry, rz, ax, ay, az)
                            RT_chess_to_base = end_to_base @ cam2end
                            print("相機座標至末端座標值 : ",RT_chess_to_base @ [X,Y,150, 1])
                        # threading.Thread(target=PyQt_MVC_Main.move_by_cam, args=(self, X, Y)).start()
                        # trace_object = False
    ...
    def linkEvent(self):
        ...
        self.ui.import_coords.clicked.connect(self.import_csv)
   ...
   # [151.5, -0.6, 211.0, 97.28, 25.21, 92.88]
    ...
    def X_PLUS(self):
        ...
        try:
            ...
            coords = mc.get_coords()
            time.sleep(s)
            mc.send_coords([ coords[0]+step, coords[1], coords[2]+6,  0, 80,0], 50, 1)
            time.sleep(s)
    ...
    def clear_table(self):
        ...
        self.ui.excute_coord.setText('執行')
    #執行動作
    def excute_coord(self):
        global mc
        global take_picture
        try:
            if self.ui.excute_coord.text() == "執行":
                table = self.ui.tableWidget
    ...
    def import_csv(self):
        try:
            print('匯入')
            global paramater
            directory = QtWidgets.QFileDialog.getOpenFileName(self, "選取檔案", "./",
                                                              "Csv Files (*.csv);;Text Files (*.txt)")
            # print(directory)
            df = pd.read_csv(directory[0]).values
            # print(df)
            #
            table = self.ui.tableWidget
            ...
            self.ui.excute_coord.setText('執行並拍攝照片')
        except:
            print(traceback.print_exc())
            msg_box = QMessageBox(QMessageBox.Warning, '警告', '此檔案非參數Json格式')
            msg_box.exec_()
        ...
        if set_cood_for_obj:
            try:
                if X ** 2 > 5 or Y ** 2 > 5:
                    ...
                    print("當前座標 : ", coords)
                    print("當前XY : ", (X,Y))
                    print(f"補正值 Y = {coords[1] - X}補正值 Z = {coords[2] - Y}")
                    time.sleep(0.2)
                    mc.send_coords(
                        # [coords[0], coords[1], coords[2]+5 ,  coords[3] + round(X,2), coords[4] - round(Y,2), coords[5]], 50, 1)
                        [coords[0], round(coords[1] - X,2), round(coords[2] - Y,2),  coords[3]+ round(X*0.1,2), coords[4]+ round(Y*0.1,2), coords[5]], 50, 1)
                        # [coords[0]-2, round(coords[1] - X,2), round(coords[2] - Y,2),coords[3]+ round(X*0.5,2),  coords[4]+ round(Y*0.5,2) , coords[5]], 50, 1)
                    time.sleep(0.2)
